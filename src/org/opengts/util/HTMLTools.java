...
public class HTMLTools
{
    ...
    **/
    public static String getMimeTypeFromExtension(String extn, String dft)
    {
        ...
        if (extn.equalsIgnoreCase("txt") || extn.equalsIgnoreCase("out") || extn.equalsIgnoreCase("conf")) {
            return CONTENT_TYPE_PLAIN;
        } else
    ...
    **/
    public static void main(String argv[])
    {
        ...
        if (RTConfig.hasProperty(ARG_POST)) {
            try {
                URIArg uri = new URIArg(RTConfig.getString(ARG_POST,""));
                byte data[] = readPage_POST(uri.getURI(), null, uri.getArgString().getBytes(), -1);
                String s = StringTools.toStringValue(data);
                Print.sysPrintln("Response:\n");
                Print.sysPrintln(s);
                System.exit(0);
